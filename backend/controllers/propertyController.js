...
// TODO FETCH TYPE OF PROPERTIES. EX: {one bhk: 34, : 23}
propertyController.get('/find/types', async (req, res) => {
    try {
        const builderType = await Property.countDocuments({ type: 'builder' })
        const societyType = await Property.countDocuments({ type: 'society' })
        const houseType = await Property.countDocuments({ type: 'house' })

        return res.status(200).json({ builder: builderType, society: societyType, house: houseType })
    } catch (error) {
        return res.status(500).json(error)
    }
})
